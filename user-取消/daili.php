<?php
    include './head.php';
    $numrows = $DB->getColumn("SELECT count(*) FROM `pre_user` WHERE `site`='{$siteInfo['id']}");
?>
...
$offset=$pagesize*($page - 1);
	        ...
	        $site_info = $DB->query("SELECT * FROM `pre_user` WHERE `site`='{$siteInfo['id']}' ORDER BY `id` DESC limit $offset,$pagesize");
	        if($site_info){
	            while($row = $site_info->fetch()){
    			 if($row['status']==1){
                    $row['status'] = '<span onclick="" class="label label-success"><b>正常</b></span>';
                  }else{
                    $row['status'] = '<span onclick="" class="label label-secondary"><b>封禁</b></span>';
                  }
                  if($row['sex']=='1'){
                      $row['sex'] = '<span class="label label-info"><b>男</b></span>';
                  }elseif($row['sex']=='0'){
                      $row['sex'] = '<span class="label label-danger"><b>女</b></span>';
                  }else{
                      $row['sex'] = '<span class="label label-warning"><b>暂无</b></span>';
                  }
                  if(!$row['age']){
                      $row['age'] = '<span class="label label-warning"><b>暂无</b></span>';
                  }
                  if(!$row['touxiang']){
