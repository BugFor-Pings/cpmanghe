<?php
    ...
    if($user){
    	$numrows = $DB->getColumn("SELECT count(*) FROM `pre_point` where `user` like '%{$user}%'");
    }else{
    	$numrows = $DB->getColumn("SELECT count(*) FROM `pre_point`");
    }
?>
<style>
.am-form-file
...
$offset=$pagesize*($page - 1);
	if(!empty($user)){
	    $link = '&user='.$user;
	    $point_info = $DB->query("SELECT * FROM `pre_point` WHERE `user` like '%{$user}%' ORDER BY `id` DESC limit $offset,$pagesize");
	}else{
	    $point_info = $DB->query("SELECT * FROM `pre_point` ORDER BY `id` DESC limit $offset,$pagesize");
	}
	        while($row = $point_info->fetch()){
